...
// PDF and QR code dependencies commented out - route is disabled
...
router.get('/:id', async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Create PDF - disabled (dependencies not installed)
    ...
    // Pipe PDF to response - disabled
    // doc.pipe(res);
    
    // Return JSON response instead since PDF generation is disabled
    return res.json({
      success: true,
      message: 'Receipt route is currently disabled',
      booking: booking,
    });

    // PDF generation code commented out - dependencies not installed
    // Header
    ...
    // Generate QR Code - disabled (dependencies not installed)
    // try {
